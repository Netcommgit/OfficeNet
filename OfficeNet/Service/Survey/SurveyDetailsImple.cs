...
namespace OfficeNet.Service.Survey
{
    public class SurveyDetailsImple : ISurveyDetailsService
    {
        ...
        public SurveyDetailsImple(ILogger<SurveyDetailsImple> logger, ApplicationDbContext context, UserManager<ApplicationUser> userManager)
        {
            _logger = logger;
            _context = context;
                ...
                if (surveyDetails.SurveyId != null && surveyDetails.SurveyId != 0)
                {
                    if (userList.Count > 0 && surveyDetails.AuthView == 2)
                    {
                        for (int i = 0; i < userList.Count; i++)
                        {
                            ...
                            surveyAuthUser.CreatedBy = surveyDetails.CreatedBy;
        ...
        public async Task<List<SurveyList>> GetSurveyListAsync(int pageNo, int pageSize)
        {
            try
            {
                _logger.LogInformation("GetSurveyList called with PageNo={PageNo}, PageSize={PageSize}", pageNo, pageSize);

                var result = await _context.SurveyListData
        ...
        public async Task<List<UsersSurveyList>> UserSurveyList(string userId, bool IsSubmitted, int pageNo, int pageSize)
        {
            try
            {
                var result = await _context.UsersSurveyLists
        ...
        public async Task<SurveyResponse?> GetSurveyFlatResult(int surveyId, string userId)
        {
                                ...
                                Options = questionGroup
                                    ...
                                    .Select(optionGroup =>
                                    {
                                        ...
                                        return new OptionResponse
                                        {
                                            ...
                                            boolResponse = !string.IsNullOrEmpty(firstOption.ResponseText) // Assuming boolResponse is true if ResponseText is not null or empty
                                                                                                           //? new Response
                                                                                                           //{
                                                                                                           //    ResponseId = firstOption.ResponseId,
                                                                                                           //    ResponseText = firstOption.ResponseText,
                                                                                                           //    CreatedBy = firstOption.CreatedBy,
                                                                                                           //    CreatedOn = firstOption.CreatedOn
                                                                                                           //}
                                                                                                           //: null
            ...
            //                        .Select(optionGroup =>
            //                        {
            //                            var firstOption = optionGroup.FirstOrDefault(); // Safe access
        ...
        public async Task<SaveSurveyResponse> SaveSurveyFlatResultByUserIdAsync(SaveSurveyResponse surveyResponse, string userId)
        {
            ...
            foreach (var question in surveyResponse.Questions)
            {
                ...
                foreach (var option in question.Options)
                {
                    ...
                    if (option.Response != null && !string.IsNullOrEmpty(option.Response))
                    {
                        ...
                        var questionResponse = new SurveyQuestionResponse
                        {
                            ...
                            OptionId = optionId,
                            ResponseText = question.QuestionType == 1 || question.QuestionType == 2 
